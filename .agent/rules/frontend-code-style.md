...
style은 .style.ts로 구부하며 cva + 백틱으로 tailwind 클래스명이 한줄로 노출되지 않고 가독성이 높게 작성합니다.
컴포넌트에서 스타일을 사용할때는 style을 S로 임포트하고 S.{스타일변수명} 으로 사용합니다.
** 로직 테스트가 아닌 뷰를 확인해야하는 테스트는 사용자가 수동으로 테스트합니다.**
shared/components폴더 안에 있는 index만을 참조할 수 있습니다. components하위 파일은 외부파일에서 참조할 수 없습니다.
색상은 global.css에 있는 색상 변수를 사용하세요.
