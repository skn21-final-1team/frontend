...
interface PersistedReportWorkflowStore {
  notebookId: number | null
  agentMessages: Chat[]
}
...
export const useReportWorkflowStore = create<ReportWorkflowStore>()(
  persist<ReportWorkflowStore, [], [], PersistedReportWorkflowStore>(
    (set, get) => ({
      status: 'idle',
      ...
      initSession: async (notebookId) => {
        ...
        const persistedAgentMessages =
          get().notebookId === notebookId ? get().agentMessages : []

        set(createInitReportWorkflowState(notebookId, persistedAgentMessages))
          ...
          if (isReportMode()) {
            ...
            set({
              error: {
                title: '복원 실패',
                description:
                  '진행 중인 보고서 워크플로우 상태를 불러오지 못했습니다. 잠시 후 다시 시도해주세요.',
              },
            })
          }
          return
        ...
        try {
          ...
          set({
            ...
            error: {
              title: '초기화 실패',
              description:
                '리포트 워크플로우 상태를 초기화하지 못했습니다. 잠시 후 다시 시도해주세요.',
            },
          })
          return false
        }
    ...
    {
      name: 'report-workflow-store',
      storage: createJSONStorage(() => localStorage),
      partialize: (state): PersistedReportWorkflowStore => ({
        notebookId: state.notebookId,
        agentMessages: state.agentMessages,
      }),
    },
